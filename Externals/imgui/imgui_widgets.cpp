...
void ImGui::TextEx_Adat(const char* text, const ImVec2& xyoffset, const char* text_end, ImGuiTextFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    const ImVec2 text_pos(window->DC.CursorPos.x + xyoffset.x, window->DC.CursorPos.y + window->DC.CurrLineTextBaseOffset + xyoffset.y);
    const float wrap_pos_x = window->DC.TextWrapPos;
    const bool wrap_enabled = (wrap_pos_x >= 0.0f);
    if (text_end - text > 2000 && !wrap_enabled)
    {
...
void ImGui::TextColored_Adat(const ImVec4& txt_col, const ImVec2& size, const int align, const ImVec2& padding, const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    ...
    switch (align)
    {
    case 0: // Left - default : 8
        _x = padding.x;
        _y = (size.y - label_size.y) / 2;
        break;
    case 1: // Center
        _x = (size.x - label_size.x) / 2;
        _y = (size.y - label_size.y) / 2;
        break;
    case 2: // Right
        _x = size.x - padding.x;
    ...
    TextColoredV_Adat(txt_col, ImVec2(_x, _y), fmt, args);
    va_end(args);
}

void ImGui::TextColoredV_Adat(const ImVec4& txt_col, const ImVec2& offset, const char* fmt, va_list args)
{
    PushStyleColor(ImGuiCol_Text, txt_col);
    ...
    ImGuiContext& g = *GImGui;
    const char* text_end = g.TempBuffer + ImFormatStringV(g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), fmt, args);
    TextEx_Adat(g.TempBuffer, offset, text_end, ImGuiTextFlags_NoWidthForLargeClippedText);
...
bool ImGui::ButtonBehavior_Adat(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
...
void ImGui::Image_Adat(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& offset, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    ImRect bb(window->DC.CursorPos, window->DC.CursorPos + size);
    if (offset.x > 0.0) {
        bb.Min.x += (window->DC.ItemWidth + offset.x); bb.Max.x += (window->DC.ItemWidth + +offset.x); // + 105
    }
    bb.Min.y += offset.y; bb.Max.y += offset.y;
...
void ImGui::RenderRectangle_Adat(const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec2& padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    float tmp = g.Style.FramePadding.x;
    g.Style.FramePadding.x = -2;
    
    if (window->SkipItems)
        return;

    ImRect pos(window->DC.CursorPos, window->DC.CursorPos + size);
    
    const ImRect bb = pos;
        
    RenderFrame(bb.Min, bb.Max, /*col*/GetColorU32(bg_col), true, ImClamp((float)ImMin(0,0/*padding.x, padding.y*/), 0.0f, g.Style.FrameRounding));
    
    //window->DrawList->AddRectFilled(bb.Min/* + padding*/, bb.Max/* - padding*/, GetColorU32(bg_col));
        
    g.Style.FramePadding.x = tmp;
...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec2& padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ...
    ImRect pos(window->DC.CursorPos, window->DC.CursorPos + size + padding * 2);
    pos.Min.x -= 7; pos.Max.x -= 7;
    pos.Min.y -= 7; pos.Max.y -= 7;
    const ImRect bb = pos;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (render_cursor || render_selection)
    {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if(flags & ImGuiInputTextFlags_AlignRight)
                        rect = ImRect(ImVec2(frame_bb.Max.x - 2 * style.FramePadding.x - rect_size.x, rect_pos.y + bg_offy_up - g.FontSize),  ImVec2(frame_bb.Max.x - style.FramePadding.x, rect_pos.y + bg_offy_dn));
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            ImVec2 text_pos = draw_pos - draw_scroll;
            if(flags & ImGuiInputTextFlags_AlignRight)
            {
                const ImVec2 text_size_with_padding = CalcTextSize(buf_display, NULL, true, 0);
                text_pos = ImVec2(frame_bb.Max.x - text_size_with_padding.x - style.FramePadding.x, text_pos.y);
            }
...
bool ImGui::TreeNodeBehavior_Adat(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label2, bool* step_flags, ImGuiTreeNodeFlags index, ImTextureID user_texture_id, const char* label_end, const char* label2_end)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    bool *bGroup            = &step_flags[0];
    bool *bChild            = &step_flags[1];
    bool *bHasbeenedited    = &step_flags[2];
    bool *bComplete         = &step_flags[3];
    bool *bAAN              = &step_flags[4];
    bool *bActive           = &step_flags[5];
    bool *bPressed          = &step_flags[6];
    
    // We vertically grow up to current line height up the typical widget height.
    const float border = 2;
    const float header = (*bAAN) ? -5 : 25.0;
    const float offset_x = --index * 30 + header;
    const float frame_height = 22;// ImMax(ImMin(window->DC.CurrLineSize.y, g.FontSize + style.FramePadding.y * 2), label_size.y + padding.y * 2); 
    ImRect frame_bb, frame_br, interact_bb_Arrow, frame_bb_title;

    // Frame
    frame_bb.Min.x = ((flags & ImGuiTreeNodeFlags_SpanFullWidth) ? window->WorkRect.Min.x : window->DC.CursorPos.x) + offset_x;
    frame_bb.Min.y = window->DC.CursorPos.y;
    frame_bb.Max.x = window->WorkRect.Max.x - 8;
    frame_bb.Max.y = window->DC.CursorPos.y + frame_height;
    // Frame border
    frame_br.Min.x = ((flags & ImGuiTreeNodeFlags_SpanFullWidth) ? window->WorkRect.Min.x : window->DC.CursorPos.x) + offset_x - 5;
    frame_br.Min.y = window->DC.CursorPos.y - 1;
    frame_br.Max.x = window->WorkRect.Max.x + 3 - 8;
    frame_br.Max.y = window->DC.CursorPos.y + frame_height;
    // Framw Arrow
    interact_bb_Arrow.Min.x = (flags & ImGuiTreeNodeFlags_SpanFullWidth) ? window->WorkRect.Min.x : window->DC.CursorPos.x;
    interact_bb_Arrow.Min.y = frame_bb.Min.y;
    interact_bb_Arrow.Max.x = frame_bb.Min.x - 5;
    interact_bb_Arrow.Max.y = frame_bb.Max.y;
    // Frame Title
    frame_bb_title.Min.x = frame_bb.Min.x - 5;
    frame_bb_title.Min.y = frame_bb.Min.y;
    frame_bb_title.Max.x = frame_bb.Min.x + header;   // Sentence index 
    frame_bb_title.Max.y = frame_bb.Max.y;
    if (display_frame)
    {
        // Framed header expand a little outside the default padding, to the edge of InnerClipRect
        // (FIXME: May remove this at some point and make InnerClipRect align with WindowPadding.x instead of WindowPadding.x*0.5f)
    ...
    const float text_offset_x = ((20 - label_size.x) / 2) + offset_x;//g.FontSize + (display_frame ? padding.x * 3 : padding.x * 2);           // Collapser arrow width + Spacing
    const float text_offset_y = (frame_height - g.FontSize) / 2;//ImMax(padding.y, window->DC.CurrLineTextBaseOffset);                    // Latch before ItemSize changes it
    const float text_width = g.FontSize + (label_size.x > 0.0f ? label_size.x + padding.x * 2 : 0.0f);  // Include collapser
    ImVec2 text_pos(window->DC.CursorPos.x + text_offset_x, window->DC.CursorPos.y + text_offset_y);
    ItemSize(ImVec2(text_width, frame_height), padding.y);
    ...
    bool hovered, held;
    bool pressed = ButtonBehavior(interact_bb_Arrow, id, &hovered, &held, button_flags); // arrow clicked

    ImGuiButtonFlags button_flags_ = ImGuiTreeNodeFlags_None;
    bool hovered_, held_;
    if(!*bGroup)
        ButtonBehavior_Adat(interact_bb, id, &hovered_, &held_, button_flags_);  // sentence clicked
    ...
    // Render
    const ImU32 text_col = GetColorU32(ImGuiCol_Text);  // ImGuiCol_Text
    ImGuiNavHighlightFlags nav_highlight_flags = ImGuiNavHighlightFlags_TypeThin;
    if (display_frame)
    {
        // Framed type
        const ImU32 bg_col = GetColorU32(ImVec4(1.0, 1.0, 1.0, 1.0));// GetColorU32((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header);
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, bg_col, style.FrameRounding, ImDrawFlags_RoundCornersAll);

        if (!*bAAN) {
            // Title
            const ImU32 bg_col_title = GetColorU32(ImVec4(0.84, 0.84, 0.84, 1.0));
            window->DrawList->AddRectFilled(frame_bb_title.Min, frame_bb_title.Max, bg_col_title, style.FrameRounding, ImDrawFlags_RoundCornersLeft);
        }

        RenderNavHighlight(frame_bb, id, nav_highlight_flags);
        if (flags & ImGuiTreeNodeFlags_Bullet)
            RenderBullet(window->DrawList, ImVec2(text_pos.x - text_offset_x * 0.60f, text_pos.y + g.FontSize * 0.5f), text_col);
        else if (*bChild/*!is_leaf*/) {
            RenderArrow_Adat(window->DrawList, ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y), GetColorU32(ImVec4(0.64, 0.64, 0.64, 1.0)), is_open ? ImGuiDir_Up : ImGuiDir_Down, index * 30, 1.0f);
        }
        else // Leaf without bullet, left-adjusted text
            text_pos.x -= text_offset_x;
        if (flags & ImGuiTreeNodeFlags_ClipLabelForTrailingButton)
        ...
        // sentence title
        text_pos.x = window->DC.CursorPos.x + text_offset_x;
        RenderTextClipped_Adat(text_pos, frame_bb.Max, label, label_end, &label_size);

        // sentence text
        text_pos.x = window->DC.CursorPos.x + offset_x + 30;
        if (!label2_end) {
            label2_end = FindRenderedTextEnd(label2);
        }
        const ImVec2 label2_size = CalcTextSize(label2, label2_end, false);
        RenderTextClipped_Adat(text_pos, frame_bb.Max, label2, label2_end, &label2_size);
       
        // selected frame
        if ((g.ActiveId_Adat == id) && (!*bGroup) && (!*bAAN)){
            g.Style.FrameBorderSize = 1.0;
            ImVec4 c2(0.00f, 0.67f, 0.83f, 1.0f);
            RenderFrameBorder_Adat(frame_br.Min, frame_br.Max, c2, style.FrameRounding);
            g.Style.FrameBorderSize = 0.0;
            *bHasbeenedited = true;
            *bActive = true;
            if (held_) *bPressed = true;
        }
        else {
            *bActive = false;
        }

        // new sentence reminder
        if ((!*bGroup) && (!*bAAN) && (!*bHasbeenedited)){
            ImVec2 center(frame_bb.Min.x, frame_bb.Min.y + 2);
            ImU32 col = ImGui::GetColorU32(IM_COL32(0, 172, 211, 255));
            window->DrawList->AddCircleFilled(center, 4.0f, col, 12);
        }

        // complete status
        if (user_texture_id) {
            window->DC.CursorPos.y = window->DC.CursorPosPrevLine.y + 2;
            ImGui::Image_Adat(user_texture_id, ImVec2(20, 20), ImVec2(100, 0), ImVec2(0, 1.0), ImVec2(1.0, 0.0), ImColor(1.00f, 1.00f, 1.00f, 1.00f), ImColor(0.00f, 0.00f, 0.00f, 0.00f));
        }
    }
    else
    {
    ...
    g.Style.FrameRounding = 0;  // ichinlo
...
bool ImGui::CollapsingHeader_Adat(const char* label, const char* label2, bool* bFlag, const int depth, ImTextureID user_texture_id, ImGuiTreeNodeFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;

    return TreeNodeBehavior_Adat(window->GetID(label), flags | ImGuiTreeNodeFlags_CollapsingHeader, label, label2, bFlag, depth, user_texture_id);
}
